...
format(object.size(all_ha), units = "Mb")
...
format(object.size(n_ha2), units = "Mb")
...
This demonstrates the efficiency of the filter-first, then download approach: The size of the object downloaded by using `bcdc_query_geodata()` with `filter()` is `r paste0(format((1 - as.numeric(object.size(n_ha2)) / as.numeric(object.size(all_ha))) * 100, digits = 1), "%")` smaller than downloading the entire dataset using `bcdcd_get_data()` and filtering locally.
