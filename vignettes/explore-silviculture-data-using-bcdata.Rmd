...
dpg %>%
  ...
  geom_sf() +
  theme_minimal()
#> Error in ggplot(.): object 'dpg' not found
```
...
```r
dim(trees_dpg)
#> Error in eval(expr, envir, enclos): object 'trees_dpg' not found
```
...
trees_dpg %>%
  ...
  theme_minimal()
#> Error in ggplot(.): object 'trees_dpg' not found
```
...
#> Error in mutate(., age = 2020 - REFERENCE_YEAR + S_SPECIES_AGE_1): object 'trees_dpg' not found
```
...
#> Error in st_join(., bgc_dpg[, "MAP_LABEL"]): object 'trees_dpg' not found
```
...
trees_bec_dpg %>%
  ...
  theme_minimal() +
  theme(legend.position = "none")
#> Error in group_by(., MAP_LABEL): object 'trees_bec_dpg' not found
```
