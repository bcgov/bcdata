---
title: "An Analysis of Silviculture Data with bcdata"
...
This short vignette will demonstrate a few ways that a user can use `bcdata` to work with and analyze the [RESULTS silviculture data sets](https://catalogue.data.gov.bc.ca/dataset?q=silviculture&download_audience=Public&sort=score+desc%2C+record_publish_date+desc) available through the B.C. Data Catalogue.  


## Getting Started

To start, let's load `bcdata` and some other R libraries we will need:
```{r , eval = FALSE}
library(bcdata)
library(tidyverse)
library(leaflet)
