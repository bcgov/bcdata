...
```{r}
bcdc_get_geodata("bc-major-cities-points-1-2-000-000-digital-baseline-mapping", LONG_TYPE == 'VILLAGE')
```
Note that because 'VILLAGE' is a character, it needs to be enclosed in single quotes. If we wanted to chose multiple values of `LONG_TYPE` we can again write it as we normally would in R using the `%in%` operator:

```{r}
bcdc_get_geodata("bc-major-cities-points-1-2-000-000-digital-baseline-mapping", LONG_TYPE %in% c('CITY','VILLAGE'))
```
