...
```{r, include=FALSE}
knitr::include_graphics("./vignettes/bcdc_browse.png")
```
...
`bcdc_get_data()` will also detect if the data resource is a geospatial file, and automatically reads and returns it in as an [`sf` object](https://r-spatial.github.io/sf/) in your R session. Let's get the air zones for British Columbia:
...
## Look at the air zone geospatial data with ggplot()
bc_az %>% 
  ggplot() +
  geom_sf()
```
...
Many geospatial datasets in the B.C. Data Catalogue are available as a [Web Map or Web Feature Service (WMS/WFS)](https://www2.gov.bc.ca/gov/content?id=95D78D544B244F34B89223EF069DF74E). While `bcdc_get_data()` will retrieve the WMS/WFS geospatial data automatically from the web for you (and return it as an `sf` object), sometimes the geospatial file is very large--and slow to download---and you may only want _some_ of the data. `bcdc_query_geodata()` let's you query catalogue geospatial data available as WMS/WFS using `select` and `filter` functions (just like in [`dplyr`](https://dplyr.tidyverse.org/), returning the query results as an [`sf` object](https://r-spatial.github.io/sf/) in your R session.

Let's get the Regional Districts for British Columbia
...
bcdc_describe_feature("d1aff64e-dbfe-45a6-af97-582b7f6418b9")
# bcdc_describe_feature(bc_regional_districts_metadata)

my_regional_district <- bcdc_query_geodata("d1aff64e-dbfe-45a6-af97-582b7f6418b9") %>% 
  filter(ADMIN_AREA_NAME == "Capital Regional District") %>% 
  collect()

## Look at the geospatial data with ggplot()
...
Please see the vignette("Querying spatial data with bcdata") for a demonstration on how to use `bcdata::bcdc_query_geodata()` to fine tune a Web Map or Web Feature Service (WMS/WFS) request for geospatial data from the B.C. Data Catalogue.
