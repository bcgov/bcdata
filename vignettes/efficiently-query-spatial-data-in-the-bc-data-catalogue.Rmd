...
A second approach is to set an internal option (`max_geom_pred_size`) and increase the threshold of when a bounding box is drawn. Options are set in R like this:
```{r, eval=FALSE}
options("max_geom_pred_size" = {object size in bytes})
```

The value of `max_geom_pred_size` is set consertivately so that requests to the Web Service are more consistently successful. Increasing this value may result in invalid requests. 
