...
test_that("Different combinations of predicates work", {
  ...
  expect_equal(
    as.character(cql_translate(WITHIN(the_bbox))),
...
test_that("subsetting works locally", {
  ...
  expect_equal(
    as.character(cql_translate(foo == x[1], .colnames = "foo")),
    "(\"foo\" = 'a')"
  )
  expect_equal(
    as.character(cql_translate(foo %in% local(y$id), .colnames = "foo")),
    "(\"foo\" IN ('a', 'b'))"
  )
  expect_equal(
    as.character(cql_translate(foo %in% y[["id"]], .colnames = "foo")),
    "(\"foo\" IN ('a', 'b'))"
  )
  expect_equal(
    as.character(cql_translate(foo == local(y$id[2]), .colnames = "foo")),
    "(\"foo\" = 'b')"
  )
})
...
test_that("large vectors supplied to filter succeeds", {
  ...
  expect_s3_class(
    bcdc_query_geodata(lines_record) %>%
...
test_that("multiple filter statements are additive", {
  ...
  expect_identical(
    finalize_cql(heliports_one_line$query_list$CQL_FILTER),
...
test_that("multiple filter statements are additive with geometric operators", {
  ...
  expect_equal(
    as.character(finalize_cql(em_program$query_list$CQL_FILTER)),
    cql_query
  )
})
...
test_that("an intersect with an object greater than 5E5 bytes automatically gets turned into a bbox", {
  ...
  expect_message(
    parks <- bcdc_query_geodata(
      record = "6a2fea1b-0cc4-4fc2-8017-eaf755d516da"
    ) %>%
...
test_that("an intersect with an object less than 5E5 proceeds", {
  skip_on_cran()
  skip_if_net_down()
  small_districts <- bcdc_query_geodata(
...
test_that("Using BBOX works", {
  ...
  query <- bcdc_query_geodata(
    ...
    filter(BBOX(
      c(1639473.0, 528785.2, 1665979.9, 541201.0),
      crs = "EPSG:3005"
    )) %>%
    show_query()
  expect_equal(
    query$query_list$CQL_FILTER,
    structure(
      "(BBOX(GEOMETRY, 1639473, 528785.2, 1665979.9, 541201, 'EPSG:3005'))",
...
test_that("Nesting functions inside a CQL geometry predicate works (#146)", {
  ...
  expect_match(
    as.character(qry2$query_list$CQL_FILTER),
  ...
  expect_error(
    suppressWarnings(
      bcdc_query_geodata("local-and-regional-greenspaces") %>%
        filter(DWITHIN(
          st_buffer(the_geom, 10000, nQuadSegs = 2),
          100,
          "meters"
        ))
    ),
